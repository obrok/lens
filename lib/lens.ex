defmodule Lens do
  ...
  @doc """
      iex> Lens.context(Lens.keys([:a, :c]), Lens.key(:b) |> Lens.all()) |> Lens.to_list(%{a: %{b: [1, 2]}, c: %{b: [3]}})
      [{%{b: [1, 2]}, 1}, {%{b: [1, 2]}, 2}, {%{b: [3]}, 3}]
      iex> Lens.context(Lens.keys([:a, :c]), Lens.key(:b) |> Lens.all()) |> Lens.map(%{a: %{b: [1, 2]}, c: %{b: [3]}}, fn({%{b: bs}, value}) ->
      ...>   length(bs) + value
      ...> end)
      %{a: %{b: [3, 4]}, c: %{b: [4]}}
  """
